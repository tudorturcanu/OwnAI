...
public final class KokoroTTS {
  ...
  public func generateAudio(voice: MLXArray, language: Language, text: String, speed: Float = 1.0) throws -> ([Float], [MToken]?) {
    ...
    return (audio.squeezed(axis: 0).asArray(Float.self), tokenArray)
  }
  ...
  private func extractStyleEmbeddings(from voice: MLXArray, tokenCount: Int) -> (MLXArray, MLXArray) {
    // Extract reference style from voice embedding. A range on the first
    // axis keeps this a slice (no gather; see predictDurations).
    let referenceStyle = voice[(tokenCount - 1) ..< tokenCount, 0 ... 1, 0...].squeezed(axis: 0)
  ...
  private func predictDurations(features: MLXArray, batchSize: Int, speed: Float) -> (MLXArray, MLXArray) {
    ...
    // `.squeezed(axis: 0)` rather than `[0]`: an integer subscript is a gather
    // with a 0-dim index, which MLX's Metal kernel feeds to setBytes as an
    // empty buffer and Xcode's Metal API validation aborts on.
    let durationSigmoid = MLX.sigmoid(durationLogits).sum(axis: -1) / speed
    let predictedDurations = MLX.clip(durationSigmoid.round(), min: 1).asType(.int32).squeezed(axis: 0)
  ...
  private func createAlignmentTarget(durations: MLXArray, batchSize: Int) -> MLXArray {
    // Pull the durations to the CPU once. Indexing the MLXArray element by
    // element (`durations[i].item()`) issues one GPU gather per phoneme, which
    // is slow and — with Metal API validation on in debug builds — aborts
    // inside MLX's Gather kernel with "setBytes: bytes argument cannot be nil".
    let frameCounts = durations.asArray(Int32.self).map { Int($0) }
    let totalFrames = frameCounts.reduce(0, +)

    // One-hot alignment matrix: row = phoneme, column = frame.
